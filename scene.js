...
for(let p of this.people.values()){let arrived=p.destination&&Math.hypot(p.x-p.destination.x,p.y-p.destination.y)<3;p.arrived=!!arrived;if(arrived&&!p.coffee)p.facing='up';p.activity=arrived?p.plannedActivity:p.coffee?'indo à máquina de café':'a caminho';p.chat=null;p.partner=null;}
...
function chairSpot(o,from){const{x,y,w,h,type}=o;const nearest=list=>!from?list[0]:list.reduce((a,b)=>Math.hypot(b.x-from.x,b.y-from.y)<Math.hypot(a.x-from.x,a.y-from.y)?b:a);if(type==='reception')return{x:x+40,y:y+h-3};if(['desk','executive','console'].includes(type))return{x:x+w*.5,y:y+h+18};if(type==='cafetable')return nearest(chairSeats(o));if(type==='armchair')return{x:x+w/2,y:y+h/2+5};if(type==='sofa')return nearest([{x:x+w*.27,y:y+h*.66},{x:x+w*.73,y:y+h*.66}]);return null}
