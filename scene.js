...
function unstick(p,people){p.stuckStreak=(p.stuckStreak||0)+1;let fresh=path(p,p.destination,people);if(fresh.length){p.route=fresh;p.stuckStreak=0;p.lastDetour=null;return}const opposite={r:'l',l:'r',d:'u',u:'d'};let alternatives=[{x:p.x+70,y:p.y,dir:'r'},{x:p.x-70,y:p.y,dir:'l'},{x:p.x,y:p.y+70,dir:'d'},{x:p.x,y:p.y-70,dir:'u'}].filter(q=>clear(q.x,q.y)&&q.dir!==opposite[p.lastDetour]&&people.every(b=>Math.hypot(q.x-b.x,q.y-b.y)>=26));alternatives.sort((a,b)=>Math.hypot(a.x-p.destination.x,a.y-p.destination.y)-Math.hypot(b.x-p.destination.x,b.y-p.destination.y));let detour=alternatives[0];if(detour){p.lastDetour=detour.dir;p.route=path(p,detour,people).concat(path(detour,p.destination,people));}if(p.stuckStreak>3){let escape=gridPoint(p,activeFloor,people,26);p.route=path(p,escape,people).concat(path(escape,p.destination,people));p.stuckStreak=0;p.lastDetour=null;}}
