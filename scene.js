...
// Mesma base/moldura do monitor, mas virado para quem olha de fora — painel traseiro fosco, nervura
// central e um cabo a sair por trás, sem ecrã aceso. Para postos onde a pessoa fica de frente para
// quem chega (a tela dela aponta para dentro, não para o balcão).
function monitorBack(c,cx,baseY,w){
 const h=w*.66,top=baseY-w*.2-h;
 ellipse(c,cx,baseY,w*.24,w*.065,'#2b3843');round(c,cx-w*.06,baseY-w*.22,w*.12,w*.24,2,'#46555f');
 round(c,cx-w/2,top,w,h,3,'#26333d');round(c,cx-w/2+2,top+2,w-4,h-5,2,'#313f49');
 round(c,cx-w*.08,top+h*.18,w*.16,h*.64,2,'#26333d');
 line(c,[[cx+w*.32,baseY-w*.14],[cx+w*.46,baseY+3]],'#1d272f',1.4);
}
